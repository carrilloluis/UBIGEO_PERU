...
/**
  ...
  * @date 		04.JULIO.2023
  * @version 	0.0.1.0
  * @about		Módulo de CÓDIGOS de UBIGEO [RENIEC] (servicio para FRONTEND) / Listado de PROVINCIAS

  */

function LIST_PROVINCES_BY_DEPARTMENT (
	$pIdDepartment_ /* ID (2 chars) del DEPARTAMENTO */
)
{
	...
	*/
	$strQuery = "SELECT DISTINCT SUBSTR(`Id`, 3, 2) AS code, `Province` AS title FROM `Survey_Places` WHERE LENGTH(`Id`) = 6 AND SUBSTR(`Id`, 1, 2) = ? ORDER BY `Id`
	"; # GROUP BY `Id`
